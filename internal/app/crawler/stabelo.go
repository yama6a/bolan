...
func (c *StabeloCrawler) Crawl(channel chan<- model.InterestSet) {
	...
	// Print summary table
	c.printRatesTable(allRates)
}

// printRatesTable prints a formatted table of all crawled rates.
func (c *StabeloCrawler) printRatesTable(rates []model.InterestSet) {
	if len(rates) == 0 {
		fmt.Println("\n=== Stabelo Rates: No rates found ===")
		return
	}

	fmt.Println("\n" + strings.Repeat("=", 70))
	fmt.Println("                        STABELO MORTGAGE RATES")
	fmt.Println(strings.Repeat("=", 70))

	// Group rates by type
	ltvRates, avgRates := groupStabeloRatesByType(rates)

	// Print LTV Discounted Rates
	if len(ltvRates) > 0 {
		printRateSection("LTV Discounted Rates (≤60% LTV tier)", ltvRates)
	}

	// Print Average Rates summary
	if len(avgRates) > 0 {
		printAvgRatesSummary(avgRates)
	}

	fmt.Println("\n" + strings.Repeat("=", 70))
	fmt.Printf("  Total: %d rates crawled (%d LTV, %d average)\n", len(rates), len(ltvRates), len(avgRates))
	fmt.Println(strings.Repeat("=", 70))
}

func groupStabeloRatesByType(rates []model.InterestSet) (ltvRates, avgRates []model.InterestSet) {
	for _, r := range rates {
		switch r.Type {
		case model.TypeRatioDiscounted:
			ltvRates = append(ltvRates, r)
		case model.TypeAverageRate:
			avgRates = append(avgRates, r)
		case model.TypeListRate, model.TypeUnionDiscounted:
			// Not used by Stabelo crawler
		}
	}
	return ltvRates, avgRates
}

func printRateSection(title string, rates []model.InterestSet) {
	fmt.Printf("\n  %s:\n", title)
	fmt.Println("  " + strings.Repeat("-", 40))
	fmt.Printf("  %-15s %15s\n", "Term", "Rate")
	fmt.Println("  " + strings.Repeat("-", 40))
	for _, r := range rates {
		fmt.Printf("  %-15s %14.2f%%\n", r.Term, r.NominalRate)
	}
}

func printAvgRatesSummary(avgRates []model.InterestSet) {
	// Group rates by month
	ratesByMonth := groupRatesByMonth(avgRates)

	// Sort months in reverse chronological order
	months := sortMonthsDescending(ratesByMonth)

	// Extract unique terms in order from the data
	terms := extractUniqueTerms(avgRates)

	fmt.Printf("\n  Average Rates (Historical) - %d months:\n", len(months))
	...
	for _, monthKey := range months {
		rates := ratesByMonth[monthKey]
		if len(rates) == 0 {
			continue
		}
		m := rates[0].AverageReferenceMonth
		fmt.Printf("  %-20s", fmt.Sprintf("%s %d", m.Month, m.Year))
		printMonthRatesRow(rates, terms)
	}
}

func groupRatesByMonth(avgRates []model.InterestSet) map[string][]model.InterestSet {
	result := make(map[string][]model.InterestSet)
	for _, r := range avgRates {
		if r.AverageReferenceMonth == nil {
			continue
		}
		key := fmt.Sprintf("%04d-%02d", r.AverageReferenceMonth.Year, r.AverageReferenceMonth.Month)
		result[key] = append(result[key], r)
	}
	return result
}

func sortMonthsDescending(ratesByMonth map[string][]model.InterestSet) []string {
	months := make([]string, 0, len(ratesByMonth))
	for k := range ratesByMonth {
		months = append(months, k)
	}
	// Sort descending (newest first) using simple bubble sort
	for i := range len(months) - 1 {
...
func printMonthRatesRow(rates []model.InterestSet, terms []model.Term) {
	// Build a map of term -> rate
	termRates := make(map[model.Term]float32)
	for _, r := range rates {
		termRates[r.Term] = r.NominalRate
	}

	for _, term := range terms {
		if rate, ok := termRates[term]; ok {
			fmt.Printf("%-12.2f", rate)
		} else {
			fmt.Printf("%-12s", "-")
		}
	}
...
// extractUniqueTerms extracts unique terms from rates in the order they appear.
func extractUniqueTerms(rates []model.InterestSet) []model.Term {

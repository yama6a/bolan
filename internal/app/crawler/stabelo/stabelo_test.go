...
func TestStabeloCrawler_Crawl(t *testing.T) {
	...
	rateTableHTML := crawlertest.LoadGoldenFile(t, "testdata/stabelo_rate_table.html")
	logger := zap.NewNop()
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.expectSomeRates && len(results) > 0 {
				crawlertest.AssertBankName(t, results, stabeloBankName)
			}
		})
	}
}
...
func TestStabeloCrawler_InterfaceCompliance(t *testing.T) {
	...
	var _ crawlertest.SiteCrawler = &StabeloCrawler{}
}
